...
long double fsita(long double tt, long double sitaa, long double rr){
	...
	output=-(A/(B*rr))*(pai*sin(2*pai*rr/R0)*sin(m*sitaa+w*tt+k*rr)/R0  +  k*sin(pai*rr/R0)*sin(pai*rr/R0)*cos(m*sitaa+w*tt+k*r));
	return output;
}
...
int main(){
    ...
    cout<<"！！！此版本 已经给定R0=5，m=5，fai=0，A=1，B=1, w=1 ,k=1"<<endl;
    R0=5.0,m=5.0,fai=0.0,A=1.0,B=1.0,w=1.0,k=1.0;
