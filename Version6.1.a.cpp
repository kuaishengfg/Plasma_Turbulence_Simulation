...
//R0 :outer edge
//Rm :inner edge
long double sita [100000], r [100000], r1 [100000] [8], r2 [100000] [9], MT [100000] [5], flux[3] [9];
...
long double m =5.0, fai = 0.0, A = 200, B = 1.0, h = 0.0001, n, k, w = 50, t;
//other var
//m  :
//fai:
//A  :
...
int main() {
	...
	for(int i=0; i < (totalnumber * 4); i++){
		...
		if (i < totalnumber){MT [i] [1] = output;}
			...
			if (i < (totalnumber * 2)){MT [i-100000] [2] = output;}
			else{
	...
	for (int i = 0; i < n; i++){
		...
		for(int ii=0; ii < totalnumber; ii++){
			...
			//judge every 100 h
			if (((i + 1) * 100) % hh == 0){
				if (MT [((ii + 1) +(i + 1) * 3) % 100000] [3]>= judgement){
					//without crash//
					RG (sitam, rm, t);
				}
				else{
					...
					//random crash;
					crashsita = MT [((ii + 1) +(i + 1) * 3) % 100000] [4] * 2 * pai;
					crashr    = MT [((ii + 1) +(i + 1) * 3) % 100000] [5] * R0 / 100;
		...
		if(((i + 1) * 10) % hh == 0) {
			...
			for(int ii = 0; ii < testnumber; ii++){
				...
				cout << "     Rr = " << Rr <<"     flux+ = " << flux [1] [ii] << "     flux- = " << flux [2] [ii] << "     flux = " << flux [0] [ii];
				outputFile << "     Rr = " << Rr <<"     flux+ = " << flux [1] [ii] << "     flux- = " << flux [2] [ii] << "     flux = " << flux [0] [ii] << endl;
				for(int i = 0; i<3; i++){
					flux [i] [ii] = 0;
				}
				Rr = Rm;
